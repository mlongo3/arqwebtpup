...
function isAutho(req,res,next){			
	...
	User.getUsuario(req.user, (usr) => {
		...
		if(!usr.habilitado){
			...
			//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			//SI NO TIENE ALQUILER HACE CRASH!!!!
			//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			...
			//Verifico que el alquiler este en fecha y que no sea admin.
			if(usr.alquiler.fechaValidez < Date.now() && usr.role != 'admin' ){ // 
				console.log('Alquiler vencido')
				Record.newRegistro(req.user,req.url,req.method,false,'advertencia','El alquiler está vencido',404)
				res.status(404).send({message: 'El alquiler está vencido'})		
			}
			else{
				if(usr.role == 'error') {
